...
import { getWelcomeMessage, saveWelcomeMessage, DEFAULT_WELCOME_MESSAGE } from "@/lib/supabase-storage";
import { Loader2, Save, RotateCcw, CheckCircle, X, MessageCircle } from "lucide-react";

interface Props {
  onClose: () => void;
}

const WelcomeMessageEditor = ({ onClose }: Props) => {
  ...
  useEffect(() => {
    getWelcomeMessage().then((t) => { setText(t); setLoading(false); });
  }, []);

  const handleSave = async () => {
    setSaving(true);
    const ok = await saveWelcomeMessage(text.trim() || DEFAULT_WELCOME_MESSAGE);
    setSaving(false);
    if (ok) {
      setSaved(true);
  ...
  const handleReset = () => {
    if (confirm("לאפס לטקסט ברירת המחדל?")) {
      setText(DEFAULT_WELCOME_MESSAGE);
      setSaved(false);
    }
  };
      ...
      <div className="bg-card rounded-2xl shadow-xl max-w-xl w-full max-h-[92vh] flex flex-col" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-heading font-bold">עריכת הודעת ווטסאפ</h2>
              <p className="text-xs text-muted-foreground">הטקסט נשלח להורה ולתלמיד עם קוד הכניסה האישי</p>
            </div>
          </div>
