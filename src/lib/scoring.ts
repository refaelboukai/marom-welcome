...
export function generateGASGoals(scores: ScoreResults): GASGoal[] {
  ...
  if (scores.learningCharacteristics.normalized >= 0 && scores.learningCharacteristics.normalized < 3.0) {
    goals.push({
      id: "gas_lc",
      area: "מאפייני למידה",
      current: "התלמיד מתקשה בהתארגנות, זיכרון עבודה או ויסות במהלך הלמידה",
      level0: "התלמיד משתמש בכלי תיווך אחד (רשימה/לוח זמנים) באופן עקבי",
      level1: "התלמיד מתארגן עצמאית למשימות לימודיות ומווסת את עצמו ברוב הזמן",
      level2: "התלמיד מנהל את תהליך הלמידה שלו באופן יעיל ויודע לבקש סיוע מותאם",
    });
  }

  // Ensure at least 4 goals — fill from lowest-scoring domains
  const fallbackPool: GASGoal[] = [
    {
      id: "gas_se_f",
      area: "מסוגלות עצמית — חיזוק והרחבה",
      current: "מצוין צורך בהמשך חיזוק תחושת המסוגלות והאמונה ביכולת",
      level0: "התלמיד מנסח לפחות שני תחומים בהם הוא חש מסוגל",
      level1: "התלמיד מציב לעצמו יעד אישי ועוקב אחר ההתקדמות אליו",
      level2: "התלמיד משמש דוגמה ועזרה לחבר נוסף בתחום שבו הוא חזק",
    },
    {
      id: "gas_loc_f",
      area: "מיקוד שליטה — הרחבה",
      current: "ניתן להעמיק את תחושת השליטה והאחריות האישית",
      level0: "התלמיד מזהה אחריות אישית בשלושה אירועים שונים מהשבוע",
      level1: "התלמיד מתכנן צעדים מעשיים להשגת מטרה ומבצע אותם",
      level2: "התלמיד מקבל החלטות מורכבות בעצמאות ומקבל אחריות על תוצאותיהן",
    },
    {
      id: "gas_cf_f",
      area: "גמישות קוגניטיבית — הרחבה",
      current: "ניתן להעמיק את היכולת לראות פרספקטיבות שונות",
      level0: "התלמיד מציע שתי דרכי פתרון לפחות במצב מאתגר",
      level1: "התלמיד מתאים את התנהגותו לשינויים בלוז או בדרישות",
      level2: "התלמיד מסייע לחברים לשקול פרספקטיבות שונות בקונפליקט",
    },
    {
      id: "gas_qol_f",
      area: "איכות חיים — הרחבה",
      current: "ניתן להעצים אזורי חוזקה ולהרחיב מעגלי תמיכה",
      level0: "התלמיד משתתף בפעילות חברתית/לימודית משמעותית אחת בשבוע",
      level1: "התלמיד מזהה דמות תומכת ופונה אליה בעת הצורך",
      level2: "התלמיד מדווח על תחושת שייכות וסיפוק עקביים",
    },
    {
      id: "gas_lc_f",
      area: "מאפייני למידה — הרחבה",
      current: "ניתן לחזק אסטרטגיות למידה ויסות עצמי",
      level0: "התלמיד משתמש באסטרטגיית למידה אחת חדשה באופן עקבי",
      level1: "התלמיד מזהה מתי הוא צריך הפסקה/ויסות ופועל בהתאם",
      level2: "התלמיד מנהל את סדר היום הלימודי שלו ביעילות",
    },
  ];
  const usedIds = new Set(goals.map((g) => g.id.replace(/_f$/, "")));
  for (const fb of fallbackPool) {
    if (goals.length >= 4) break;
    if (!usedIds.has(fb.id.replace(/_f$/, ""))) {
      goals.push(fb);
      usedIds.add(fb.id.replace(/_f$/, ""));
    }
  }
