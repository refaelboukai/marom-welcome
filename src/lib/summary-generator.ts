...
function getPositiveInterpretation(score: number, domain: string, name: string): string {
  const map: Record<string, { high: string; mid: string; growing: string; emerging: string }> = {
    qualityOfLife: {
      high: `${name} מביע/ה שביעות רצון ותחושת רווחה בתחומי חיים מגוונים — חברתי, רגשי ולימודי. זהו בסיס חשוב להמשך צמיחה.`,
      mid: `${name} מזהה תחומים בהם מרגיש/ה טוב לצד תחומים שבהם שואף/ת להתפתח. הליווי מאפשר חיזוק המשאבים הקיימים.`,
      growing: `${name} נמצא/ת בתהליך של בניית תחושת רווחה. עם ליווי מתאים וחוויות חיוביות, ניתן לחזק את תחושת השייכות והסיפוק.`,
      emerging: `${name} מתחיל/ה לזהות את הצרכים שלו/ה בתחומי חיים שונים. זוהי נקודת מוצא חשובה לבניית תכנית ליווי אישית.`,
    },
    selfEfficacy: {
      high: `${name} מאמין/ה ביכולתו/ה להתמודד עם אתגרים ולהצליח. תחושת מסוגלות זו מהווה משאב משמעותי לקידום בכל תחום.`,
      mid: `${name} מפתח/ת את תחושת המסוגלות ומתחיל/ה לזהות חוזקות אישיות. חיזוק חוויות הצלחה יתמוך בהמשך ההתקדמות.`,
      growing: `${name} נמצא/ת בדרך לבניית ביטחון ביכולותיו/ה. משימות הדרגתיות עם חוויות הצלחה יסייעו לחזק את תחושת המסוגלות.`,
      emerging: `${name} בתחילת הדרך של גילוי היכולות. ליווי אישי המדגיש הצלחות קטנות יבנה בהדרגה את הביטחון העצמי.`,
    },
    locusOfControl: {
      high: `${name} חש/ה שיש לו/ה השפעה על מה שקורה ומקשר/ת בין מאמץ לתוצאות. תפיסה זו מחזקת מוטיבציה ואחריות אישית.`,
      mid: `${name} מתחיל/ה לזהות קשרים בין הפעולות שלו/ה לתוצאות. חיזוק תחושת השליטה יתמוך ביוזמה עצמית ובמוטיבציה.`,
      growing: `${name} בתהליך של פיתוח תחושת שליטה על חייו/ה. מתן הזדמנויות לבחירה וקבלת החלטות יחזק את תחושת ההשפעה.`,
      emerging: `${name} לומד/ת לזהות את ההשפעה שלו/ה על סביבתו/ה. ליווי שמדגיש קשר בין מאמץ לתוצאה יבנה תחושת משמעות ושליטה.`,
    },
    cognitiveFlexibility: {
      high: `${name} מגלה יכולת לחשוב על חלופות ולהסתגל למצבים שונים. גמישות זו מהווה כלי חשוב להתמודדות יעילה.`,
      mid: `${name} מפתח/ת את יכולת החשיבה הגמישה ולומד/ת לשקול אפשרויות שונות. תרגול ממוקד יחזק מיומנות חשובה זו.`,
      growing: `${name} בתהליך של הרחבת דרכי החשיבה. חשיפה למצבים מגוונים ותרגול פתרון בעיות יתמכו בפיתוח הגמישות.`,
      emerging: `${name} מתחיל/ה להכיר דרכי חשיבה חדשות. ליווי שמעודד הסתכלות ממספר זוויות יסייע בפיתוח חשיבה יצירתית ופתוחה.`,
    },
  };
...
export function generateSemesterSummary(
  ...
  for (const t of trends) {
    ...
    summary += `   ${getPositiveInterpretation(t.latest, t.key, session.studentName)}\n`;
  ...
  if (validTrends.length > 0) {
    ...
    summary += `${"─".repeat(40)}\n`;
    if (avgChange > 0.2) {
      summary += `📈 סיכום: ניכרת מגמת התקדמות חיובית. ${session.studentName} מגלה צמיחה והתפתחות.\n`;
    } else if (avgChange > 0) {
      summary += `📈 סיכום: נראים ניצנים של התקדמות. המשך הליווי יתמוך בהעמקת השינוי של ${session.studentName}.\n`;
    } else if (avgChange > -0.2) {
      summary += `📈 סיכום: ${session.studentName} שומר/ת על יציבות. ניתן להתמקד בתחומים ספציפיים לקידום.\n`;
    } else {
