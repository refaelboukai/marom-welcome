...
import { Plus, Users, AlertTriangle, CheckCircle, Clock, Search, LogOut, XCircle, Loader2 } from "lucide-react";
      ...
      <div className="bg-card border-b border-border px-4 py-3 sticky top-0 z-20">
        <div className="max-w-6xl mx-auto flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <button onClick={() => navigate("/admin/new")} className="btn-intake bg-primary text-primary-foreground text-sm px-4 py-2">
              <Plus className="w-4 h-4 inline ml-1" />
              קליטה חדשה
            </button>
            <button onClick={() => navigate("/")} className="p-2 rounded-lg hover:bg-muted transition-colors" title="יציאה">
        ...
        {filtered.length === 0 ? (
          ...
          <>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {filtered.map((session) => {
                    ...
                    return (
                      <tr key={session.id} onClick={() => navigate(`/admin/student/${session.id}`)} className="border-t border-border hover:bg-muted/30 cursor-pointer transition-colors">
                        ...
                        <td className="px-4 py-3 text-xs text-muted-foreground">{new Date(session.updatedAt).toLocaleDateString("he-IL")}</td>
                      </tr>
                    );
                  })}
                </tbody>
            ...
            <div className="md:hidden space-y-2">
              {filtered.map((session) => (
                <button key={session.id} onClick={() => navigate(`/admin/student/${session.id}`)} className="intake-card-soft w-full text-right hover:shadow-md transition-shadow">
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-primary/10 flex items-center justify-center flex-shrink-0">
                      <span className="text-primary font-bold text-sm">{session.studentName.charAt(0)}</span>
                    </div>
